...
- `ts_code` 格式（非 6 位数字）：沪市 `600519.SH` / 深市 `000333.SZ`（创业板 `300xxx.SZ`）/ 科创板 `688xxx.SH` / 北交所 `8xxxxx.BJ` 与 `920xxx.BJ`（920 号段）/ 港股 `00700.HK`。6 位代码需按交易所补后缀。
...
- **PE 三口径只有两档**：tushare 只给 `pe`(静态) 与 `pe_ttm`(TTM)，**没有腾讯源的「PE动/前瞻(年化)」**——做前瞻 PE 的分母用下方 `fetch_forecast.py` 的机构一致预期净利润均值折算，勿硬找"动态 PE"。
- **港股限制**：fetch_quotes.py 港股走 `hk_daily`（仅价/量），**缺 PB/PE/市值/股息率**等估值字段；港股估值指标退回下方腾讯源，前瞻口径同样退回腾讯源 PE(动)（索引 52）或如实标注缺口。
- 财务同比/毛利率/ROE 类字段**不在快照输出里**：ROE/增速的历史序列可取 `fetch_forecast.py` 的「详细指标预测」表；其余退回下方东财源（含经营同比），或另行调 tushare `fina_indicator`（由报告期驱动，取最新已披露的 `end_date` 一行即可）。

## 机构一致预期：fetch_forecast.py（同花顺 F10 盈利预测页）

前瞻PE/PEG 的分母（机构预测净利润/EPS 均值）tushare 没有，用 `python scripts/fetch_forecast.py <代码>` 取（脚本自包含纯标准库、绕过系统代理直连、**无需 token**、只读不写；工作区没有该脚本时从插件端点 `/plugins/advisor-agent/assets/workspace-init/fetch_forecast.py` 下载）。输出为小体量表格，可直接进上下文：

| 输出表 | 内容 | 估值用途 |
|---|---|---|
| 净利润一致预期（亿元） | 年度 / 机构数 / 最小 / 均值 / 最大 / 行业平均 | 前瞻PE、PEG、隐含增速的分母 |
| 每股收益一致预期（元） | 同上 | 前瞻 EPS、增速交叉核对 |
| 机构预测明细 | 机构 / 研究员 / 各年 EPS / 净利润 / 报告日期 | 分歧度（最大−最小）/均值、报告新鲜度 |
| 详细指标预测 | 营收/净利/增速/ROE/每股净资产/静态PE 的历史实际值 + 预测均值 | 质量溢价（ROE 稳定性）、周期中枢 |

- 用法：`python scripts/fetch_forecast.py 002851`；多只逗号分隔（默认间隔 1s 防限流）；`--mcap <总市值亿元>` 追加各预测年前瞻PE——**fetch_quotes 快照的总市值单位是万元，÷1e4 后再传**。
- 仅 A 股（沪深北，含北交所/920 段）；港股（5 位代码）无此页，脚本会明确报错。
- 无机构覆盖（小盘股常见）→ 脚本如实输出「未解析到业绩预测表格」，这是**有效结果**：估值改用历史增速外推/行业增速映射并在结论中注明口径，不编造。
- 口径提醒：均值为覆盖机构预测的算术平均，机构数 <5 家代表性弱（结论注明覆盖数）；明细表只列近年报告子集，机构总数以汇总表「预测机构数」为准；报告日期超过半年的旧预测谨慎引用；「行业平均数」是同业均值对照，不是该股预测。
