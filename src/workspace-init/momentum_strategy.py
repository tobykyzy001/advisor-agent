...
- 脚本本身**不取行情**：取数统一走 fetch_quotes.py（直连 tushare REST API，增量刷库）；
  本脚本只从本地 CSV 行情库读数计算。
...
一键回滚开关（环境变量 STRATEGY_MOMENTUM_DECISION_MODE=decision_runs）：
  ...
  写入会被下一轮当作「老仓」依据的状态（降级时输出带 [降级] 提示）。
────────────────────────────────────────────────────────────────────────────────

本地 CSV 行情库（与 w_bottom_screen.py 共享 output/quotes-store/，由 fetch_quotes.py 刷库写回）：
- 每只标的一份 CSV：output/quotes-store/<ts_code>.csv，列 trade_date,open,high,low,close,vol。
- 判定前做数据门禁：库内无数据 / 最后交易日距今超过 10 个自然日的标的视为缺口，
  fail-closed 列出缺口并提示先跑 fetch_quotes.py；历史根数由覆盖率机制兜底
  （不足 121 根的标的剔除并计入覆盖率，低于下限不出信号）。

两层式工作流（取数与计算分离，取数统一走 fetch_quotes.py）：
 1) python fetch_quotes.py --watchlist output/watchlist/watchlist.yaml --min-bars 121 --full-days 420
        —— 刷库：对照库内最后交易日增量补到最新（新票/不足 121 根的全量重取近 420 自然日），
        幂等合并写回 output/quotes-store/。
 2) python momentum_strategy.py --watchlist output/watchlist/watchlist.yaml \
...
#: 数据门禁：库内最后交易日距今超过该自然日数视为数据过期（覆盖周末与长假，与 w_bottom_screen.py 一致）
MAX_STALE_DAYS = 10
...
# ═══════════════════════════════════════════════════════════════════════════
# 主流程（--plan 仅诊断 / 默认直接读库计算）
# ═══════════════════════════════════════════════════════════════════════════


def _plan(watchlist_path: Path, store_dir: Path, history_min: int = 121) -> int:
    try:
    ...
    for it in items:
        ...
        if n >= history_min and last:
            ...
            print(f"  {it.ts_code}  待全量（库内仅 {n} 根 < {history_min}，最新 {last}）")
        else:
            print(f"  {it.ts_code}  待全量（库内无数据）")
    print()
    print("刷库命令（增量自动补到最新交易日，新票/不足历史自动全量）：")
    print(f"  python fetch_quotes.py --watchlist {watchlist_path}"
          f" --min-bars {history_min} --full-days 420")
    print("刷库完成后直接计算（无需 --data，直接读库）：")
    print(f"  python momentum_strategy.py --watchlist {watchlist_path}"
          f" --state output/momentum/state.json")
    return 0


def _run(watchlist_path: Path, state_path: Path | None, p: StrategyParams,
         out_dir: Path, write_plan: bool = True, store_dir: Path = Path("output/quotes-store")) -> int:
    try:
        items = load_watchlist(watchlist_path)
    except FileNotFoundError as e:
        print(f"[error] {e}")
    ...
    if not items:
        ...
        return 1
    pool_codes = [it.ts_code for it in items]

    # 数据门禁：读库 + 新鲜度校验（缺口 fail-closed；历史根数交给覆盖率机制兜底）
    today = date.today()
    raw: dict[str, list] = {}
    gaps: list[str] = []
    for code in pool_codes:
        rows = read_store(store_dir, code)
        if not rows:
            gaps.append(f"{code}（库内无数据）")
            continue
        last = _to_date(rows[-1]["trade_date"])
        if last is None or (today - last).days > MAX_STALE_DAYS:
            gaps.append(f"{code}（库内最新 {rows[-1]['trade_date']}，"
                        f"距今超过 {MAX_STALE_DAYS} 自然日）")
            continue
        raw[code] = rows
    if gaps:
        print("[error] 以下标的行情库数据缺失或过期，本次 fail-closed 不出信号：")
        for g in gaps:
            print(f"  - {g}")
        print(f"请先刷库：python fetch_quotes.py --watchlist {watchlist_path}"
              f" --min-bars {p.history_min} --full-days 420")
        return 1

    name_map: dict[str, str] = {it.ts_code: it.name for it in items}

    # 覆盖池 = 观察仓全部标的（读库模式，缺口已被数据门禁挡掉）
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--plan", action="store_true",
                    help="仅诊断：打印观察仓清单与库内每只状态（不取数、不计算）")
    ap.add_argument("--out-dir", default="output/momentum",
                    help="信号报告输出目录（默认 output/momentum）")
    ...
    if args.plan:
        return _plan(Path(args.watchlist), Path(args.store), history_min=args.history_min)
    ...
    return _run(Path(args.watchlist), state_path, p, Path(args.out_dir),
                write_plan=write_plan, store_dir=Path(args.store))
