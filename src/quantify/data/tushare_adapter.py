...
口径说明：
- 输入是 tushare 日线接口返回的行（每条含 ts_code / trade_date /
  open / high / low / close / vol 等字段），可来自本地 CSV 行情库
  （output/quotes-store/，由 fetch_quotes.py 直连 tushare REST 刷库写回）。
...
def rows_to_series(symbol: str, rows: Iterable[dict], market: str = "A") -> DailySeries:
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        bars.append(DailyBar(date=d, open=o, high=h, low=lo, close=c, volume=vol))
