  ...
  function buildInstruction(skill, values) {
    ...
    if (skill.id === 'w-bottom-screener') {
      ...
      return [
        ...
        `脚本打印观察仓标的清单与待取数清单：池子 ≤10 只输出单片模式，>10 只自动输出分片模式（每片约 10 只，逐片列出 shard 编号与片内 ts_code）。严格按打印的模式走下面第三步。`,
        `第三步（取数，按模式二选一）：`,
        `- 单片模式：对清单里每只 ts_code 调用 mcp__tushareMcp__daily 取近 30 个交易日日线（保留 trade_date/open/high/low/close/vol），整理成 JSON 写到 ${dataFile}，格式为 {"<ts_code>": [{"trade_date":"YYYYMMDD","open":..,"high":..,"low":..,"close":..,"vol":..}, ...], ...}；行情只写文件，不要把数据贴进你的回复。`,
        `- 分片模式（必须多 agent）：先清空 ${dataDir} 目录（防上轮残留混入），然后对打印出的每个分片各派一个子代理（subagent）并行执行。每个子代理的任务：对片内每只 ts_code 调 mcp__tushareMcp__daily 取近 30 个交易日日线（保留 trade_date/open/high/low/close/vol），整理成 {"<ts_code>": [...]} 的 JSON，用写文件工具原样写入 ${dataDir}/shard_<片号>.json；子代理的回复只允许一行回执（形如「shard 1：完成 10/10，失败 []」），不得包含任何行情内容。你身为主代理要把全部分片一次性并行派发，然后只收集回执；某片失败就原样重派一次，仍失败则记录缺口后继续。`,
        `- 若本会话（含子代理）没有可用的 tushare MCP（无法调用 mcp__tushareMcp__daily），直接如实报告"无 tushare MCP，无法取数"，不要回退 akshare、不要编造行情。`,
        `第四步：单片模式跑 python scripts/w_bottom_screen.py --watchlist ${watchlist} --data ${dataFile}${extraText}；分片模式跑 python scripts/w_bottom_screen.py --watchlist ${watchlist} --data ${dataDir}${extraText}（--data 传目录时脚本自动合并全部 *.json 分片）。`,
    ...
    if (skill.id === 'momentum-rotation') {
      ...
      return [
        ...
        `第三步：python scripts/momentum_strategy.py --watchlist ${watchlist} --plan 打印待取数清单：池子 ≤10 只输出单片模式，>10 只自动输出分片模式（每片约 10 只，逐片列出 shard 编号与片内 ts_code）。严格按打印的模式走下一步。`,
        `第四步（取数，按模式二选一）：`,
        `- 单片模式：对每只 ts_code 调用 mcp__tushareMcp__daily 取近 250 个交易日日线（start_date 不晚于 today−250，保留 trade_date/open/high/low/close/vol），整理成 JSON 写到 ${dataFile}，格式为 {"<ts_code>": [{"trade_date":"YYYYMMDD","open":..,"high":..,"low":..,"close":..,"vol":..}, ...], ...}；行情只写文件，不要把数据贴进你的回复。`,
        `- 分片模式（必须多 agent）：先清空 ${dataDir} 目录（防上轮残留混入），然后对打印出的每个分片各派一个子代理（subagent）并行执行。每个子代理的任务：对片内每只 ts_code 调 mcp__tushareMcp__daily 取近 250 个交易日日线（start_date 不晚于 today−250，保留 trade_date/open/high/low/close/vol），整理成 {"<ts_code>": [...]} 的 JSON，用写文件工具原样写入 ${dataDir}/shard_<片号>.json；子代理的回复只允许一行回执（形如「shard 1：完成 10/10，失败 []」），不得包含任何行情内容。你身为主代理要把全部分片一次性并行派发，然后只收集回执；某片失败就原样重派一次，仍失败则记录缺口后继续（覆盖率不足时脚本会 fail-closed）。`,
        `- 若本会话（含子代理）没有可用的 tushare MCP（无法调用 mcp__tushareMcp__daily），直接如实报告"无 tushare MCP，无法取数"，不要回退 akshare、不要编造行情。`,
        `第五步：单片模式跑 python scripts/momentum_strategy.py --watchlist ${watchlist} --state ${stateFile} --data ${dataFile}${extraText}；分片模式跑 python scripts/momentum_strategy.py --watchlist ${watchlist} --state ${stateFile} --data ${dataDir}${extraText}（--data 传目录时脚本自动合并全部 *.json 分片）。`,
