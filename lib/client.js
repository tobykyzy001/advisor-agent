  ...
  function buildInstruction(skill, values, config) {
    ...
    if (skill.id === 'momentum-rotation') {
      ...
      return [
        ...
        `脚本直接读行情库计算 mom20/mom120/mom60 三条动量 + 三道过滤 + 快慢双榜（buffer16 粘性、快4慢4补满）算出目标持仓（库内数据缺失或过期超过 10 自然日会 fail-closed 并提示先刷库），把完整报告打印到标准输出，并保存为 output/momentum/plan_<时间戳>.md、回写持仓状态 ${stateFile}。`,
