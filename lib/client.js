  ...
  function buildInstruction(skill, values) {
    ...
    if (skill.id === 'bili-video-summary') {
      ...
      return [
        ...
        `若 --selfcheck 报 yt_dlp / faster_whisper 为 missing，先运行 \`pip install yt-dlp faster-whisper\`（可选 \`pip install opencc-python-reimplemented\`）再重跑 --selfcheck 确认 ready=true。`,
        `第三步：python <下载到的脚本路径> "${video}" --out output/videos --models output/videos/models`,
        `该脚本已内置：B站 412 反爬、hf-mirror 镜像、关闭 Xet；whisper 模型缓存收拢到 output/videos/models/（有模型就用、没有才下载）。`,
