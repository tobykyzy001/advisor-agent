    ...
    if (skill.id === 'prosperity-picking') {
      ...
      if (isPick) {
        ...
        lines.push(
          ...
          `第六步（剔除板块必须先问）：对每个拟剔除板块，先询问用户「是否把观察仓中该板块的标的（PS=<板块id>）一并踢出观察仓？」：用户同意 → 先 python scripts/manage_watchlist.py 列出观察仓，找出带 PS=<板块id> 的条目逐个 python scripts/manage_watchlist.py rm <代码>，踢完再 rm 板块；用户不同意或暂不处理 → 只剔除板块跟踪状态（rm 板块），观察仓条目原样保留。`,
