window.__ModuleLoader__.load({ id: 'advisor-agent', factory: (require) => {
  ...
  const ADVISOR_SKILLS = [
    ...
    },
    {
      id: 'workspace-init',
      label: '初始化工作区',
      description: '在任意（空）目录一键生成三大工具 + 持仓/观察仓/个股知识库的目录骨架与清单模板',
      params: {
        target: {
          type: 'string',
          label: '目标目录（可选）',
          required: false,
          placeholder: '如 D:/my-advisor（留空则在当前工作目录）',
          hint: '在该目录生成 output/（作业回测·持仓·观察仓·W底报告）与 knowledge/（个股知识库）骨架；已存在的文件不会被覆盖',
        },
      },
    },
  ]
  ...
  function buildInstruction(skill, values) {
    ...
    for (const [k, spec] of Object.entries(skill.params)) {
      let v = values[k]
      // workspace-init 的目标目录未填时，默认落到「当前工作区的磁盘路径」，
      // 保证骨架生成在用户正在看的那个工作区目录里（而非 agent 仓库根目录）。
