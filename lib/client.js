window.__ModuleLoader__.load({ id: 'advisor-agent', factory: (require) => {
  ...
  function buildInstruction(skill, values) {
    ...
    if (skill.id === 'w-bottom-screener') {
      ...
      return [
        `执行「观察仓 W底放量筛选」三段式流程。`,
        `第一步：从 ${assetUrl} 下载 w_bottom_screen.py（用 PowerShell 的 Invoke-WebRequest 或 curl，原样保存为 UTF-8 到本地临时路径）。`,
        `若下载返回 404/失败，直接如实报告"脚本资产端点不可用"，不要跳过、不要去找别的本地路径碰运气。`,
        `第二步：python <下载到的脚本路径> --watchlist ${watchlist} --plan${extraText}`,
        ...
        `第四步：python <下载到的脚本路径> --watchlist ${watchlist} --data ${dataFile}${extraText}`,
    ...
    if (skill.id === 'momentum-rotation') {
      ...
      return [
        ...
        `第二步：从 ${assetUrl} 下载 momentum_strategy.py（用 PowerShell 的 Invoke-WebRequest 或 curl，原样保存为 UTF-8 到本地临时路径）。若下载返回 404/失败，直接如实报告"脚本资产端点不可用"，不要跳过、不要去找别的本地路径碰运气。`,
        ...
        `第四步：python <下载到的脚本路径> --watchlist ${watchlist} --state ${stateFile} --data ${dataFile}${extraText}`,
        `脚本会按 mom20/mom120/mom60 三条区间动量 + MA120趋势关 + 反追高关 + 大盘状态关，做快慢双榜排名（老仓 buffer 前 16 粘性 + 快4慢4补满）并输出目标持仓报告 output/momentum/plan_*.md、回写持仓状态 ${stateFile}。`,
        `把脚本输出的报告内容与目标持仓清单原样汇报给用户。`,
      ].join('\n')
    }
    ...
    if (skill.id === 'bili-video-summary') {
      ...
      return [
        ...
        `先给用户一句「时间预期与说明」再动手：首次运行需装依赖（yt-dlp、faster-whisper）并下载一次 whisper 模型（${model==='tiny'?'约75MB':model==='base'?'约145MB':'约460MB'}），这一步只在第一次发生、之后每台机器永久复用；转录本身约 ${model==='tiny'?'0.5~1':model==='base'?'0.6~1.2':'0.4~0.8'} 倍音频时长（cpu）。也就是说：长视频慢在所难免，但换用 base/tiny 会显著更快，且二次总结同一 BV 号直接秒出（跳过转录）。请把这段话简要转达给用户，让用户心里有数、不用干等。`,
        `严格按以下步骤执行，不要跳步，每步完成给用户一句进度更新：`,
        `第一步：从 ${assetUrl} 下载 transcribe_video.py（PowerShell 的 Invoke-WebRequest 或 curl，原样保存为 UTF-8 到本地临时路径）。`,
        `若下载返回 404/失败，直接如实报告「B站视频总结脚本资产端点不可用」，不要跳过去找别的本地路径碰运气。`,
        `第二步：在工作区根目录运行 \`python <下载到的脚本路径> --selfcheck\`。`,
        `若 --selfcheck 报 yt_dlp / faster_whisper 为 missing，先运行 \`pip install yt-dlp faster-whisper\`（可选 \`pip install opencc-python-reimplemented\`）再重跑 --selfcheck 确认 ready=true；装依赖这一步会花几分钟，属首次一次性成本。`,
        `第三步：python <下载到的脚本路径> "${video}"${modelFlag} --out output/videos --models output/videos/models`,
        `该脚本已内置：B站 412 反爬、hf-mirror 镜像、关闭 Xet；whisper 模型缓存收拢到 output/videos/models/（有模型就用、没有才下载）；脚本会按 [1/4][2/4][3/4][4/4] 打印阶段进度，把每阶段进度如实转达给用户。`,
        `脚本幂等：若 output/videos/<BV号>/transcript.txt 已存在会跳过转录直接输出「完成」；换 BV 号落在各自目录。`,
        `第四步：读取 output/videos/<BV号>/transcript.txt（连同 meta.json 的标题/UP主/时长）做后续分析。`,
        `第五步：按顺序输出结论：`,
