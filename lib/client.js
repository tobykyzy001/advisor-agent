  ...
  function buildInstruction(skill, values, config) {
    ...
    if (skill.id === 'momentum-rotation') {
      ...
      return [
        ...
        `脚本直接读行情库计算 mom20/mom120/mom60 三条动量 + 三道过滤 + 快慢双榜（buffer16 粘性、快4慢4补满）算出目标持仓（库内数据缺失或过期超过 10 自然日会 fail-closed 并提示先刷库），标准输出只打印摘要（大盘状态、目标持仓表）——「逐只动量与过滤」全池明细不打印、只写入完整报告 output/momentum/plan_<时间戳>.md，并回写持仓状态 ${stateFile}。`,
        `把标准输出中的摘要与目标持仓清单原样汇报给用户，不要自行改写或省略数字；「逐只动量与过滤」明细不要读入或整表复述，向用户提示报告 md 文件位置即可（需要逐只明细时让用户直接打开该文件）。`,
