  ...
  function buildInstruction(skill, values) {
    ...
    if (skill.id === 'workspace-init') {
      ...
      return [
        ...
        `第三步（可选，供后续 B站视频总结工具直接复用）：若用户要用 B站视频总结，继续运行 \`python scripts/setup_runtime.py --target ${target}\` 建工作区持久 .venv 并装 yt-dlp、faster-whisper（可选 --with-opencc）。此步幂等、首次较慢，非必须时不主动跑；B站工具跑时会按需自行调用。`,
    ...
    if (skill.id === 'bili-video-summary') {
      ...
      return [
        ...
        `第一步：把两个脚本下载到当前工作区 scripts/ 目录（不存在则先创建）：从 ${assetUrl} 下载 transcribe_video.py、从 ${setupUrl} 下载 setup_runtime.py（都用 PowerShell 的 Invoke-WebRequest 或 curl，原样保存为 UTF-8，每次覆盖下载保证版本与插件一致）。`,
        `若任一脚本下载返回 404/失败，直接如实报告「脚本资产端点不可用」，不要跳过去找别的本地路径碰运气。`,
        `两个脚本的用法已在后续步骤命令中完整给出（依赖与环境由第二步 setup_runtime.py 统一准备），无需阅读或分析脚本源码。`,
        `第二步（装环境，只在首次发生）：在工作区根目录运行 \`python scripts/setup_runtime.py --target .\`，它会建工作区持久 .venv 并装 yt-dlp、faster-whisper（可选加 --with-opencc）。脚本幂等：.venv 已存在、依赖已装自动跳过；带 [1/2][2/2] 进度；失败会打印排障提示。装完后用 \`.venv\\Scripts\\python.exe scripts/transcribe_video.py --selfcheck\` 确认 ready=true。`,
        `第三步：用 \`.venv\\Scripts\\python.exe scripts/transcribe_video.py "${video}"${modelFlag} --out output/videos --models output/videos/models\` 跑转录（务必用刚建好的 .venv 的 python，不要用系统裸 python，也不要用临时 --target 或 monkeypatch tempfile）。`,
