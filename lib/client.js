window.__ModuleLoader__.load({ id: 'advisor-agent', factory: (require) => {
  ...
  function buildInstruction(skill, values) {
    ...
    if (skill.id === 'w-bottom-screener') {
      ...
      const dataDir = 'output/w-bottom/quotes'
      const extras = []
      ...
      return [
        `执行「观察仓 W底放量筛选」流程。取数纪律（硬约束）：行情数据体积大，本体永不进入你的会话上下文——不回显、不粘贴、不汇总明细，一律由取数（子）代理直接写盘，脚本从磁盘读取。`,
        ...
        `脚本打印观察仓标的清单与待取数清单：池子 ≤10 只输出单片模式，>10 只自动输出分片模式（每片约 10 只，逐片列出 shard 编号与片内 ts_code）。脚本对照本地 CSV 行情库 output/quotes-store/ 给每只标注取数区间：增量（start_date=库内最后日期+1，只取尾巴）、全量（新票或库内不足历史）、免取（库内已最新）。严格按打印的模式与区间走下面第三步；若打印「全部免取」，直接在 ${dataDir} 放一个内容为 {} 的 shard_1.json 后跳到第四步。`,
        `第三步（取数，按模式二选一）：`,
        `- 单片模式：对清单里每只 ts_code 按打印的区间调用 mcp__tushareMcp__daily（增量票只取 start_date 到今天的尾巴；全量票取近 30 个交易日日线；保留 trade_date/open/high/low/close/vol），整理成 JSON 写到 ${dataFile}，格式为 {"<ts_code>": [{"trade_date":"YYYYMMDD","open":..,"high":..,"low":..,"close":..,"vol":..}, ...], ...}；行情只写文件，不要把数据贴进你的回复。`,
        `- 分片模式（必须多 agent）：对打印出的每个分片各派一个子代理（subagent）并行执行（无需清空 ${dataDir} 目录：脚本按 trade_date 幂等合并，残留分片重复合并无副作用）。每个子代理的任务：对片内每只 ts_code 按打印标注的区间调 mcp__tushareMcp__daily（增量票只取尾巴；全量票取近 30 个交易日日线；保留 trade_date/open/high/low/close/vol），整理成 {"<ts_code>": [...]} 的 JSON，用写文件工具原样写入 ${dataDir}/shard_<片号>.json；子代理的回复只允许一行回执（形如「shard 1：完成 10/10，失败 []」），不得包含任何行情内容。你身为主代理要把全部分片一次性并行派发，然后只收集回执；某片失败就原样重派一次，仍失败则记录缺口后继续。`,
        `- 若本会话（含子代理）没有可用的 tushare MCP（无法调用 mcp__tushareMcp__daily），直接如实报告"无 tushare MCP，无法取数"，不要回退 akshare、不要编造行情。`,
        `第四步：单片模式跑 python scripts/w_bottom_screen.py --watchlist ${watchlist} --data ${dataFile}${extraText}；分片模式跑 python scripts/w_bottom_screen.py --watchlist ${watchlist} --data ${dataDir}${extraText}（--data 传目录时脚本自动合并全部 *.json 分片、按日期幂等写回行情库 output/quotes-store/，再用库内全量历史判定形态）。`,
        `脚本会判定 W底+放量形态并输出命中报告 output/w-bottom/screen_*.md；把报告内容与命中清单原样汇报给用户。`,
      ].join('\n')
    }
    // momentum-rotation：对观察仓所有标的做「中期动量排名 → 目标组合」。
    // 大池子（>10 只）走分片多 agent 取数 + 落盘旁路：行情数据永不进入主会话，
    // 子代理并行取数直接写盘，脚本 --data 传目录自动合并（避免 80+ 标的时主会话 token 爆炸与串行缓慢）。
    if (skill.id === 'momentum-rotation') {
      ...
      const dataDir = 'output/momentum/quotes'
      ...
      return [
        ...
        `取数纪律（硬约束）：每只近 250 日线有数千 token，行情数据本体永不进入你的会话上下文——不回显、不粘贴、不汇总明细，一律由取数（子）代理直接写盘，脚本从磁盘读取。`,
        `严格按以下步骤执行，不要跳步：`,
        ...
        `第三步：python scripts/momentum_strategy.py --watchlist ${watchlist} --plan 打印待取数清单：池子 ≤10 只输出单片模式，>10 只自动输出分片模式（每片约 10 只，逐片列出 shard 编号与片内 ts_code）。脚本对照本地 CSV 行情库 output/quotes-store/ 给每只标注取数区间：增量（start_date=库内最后日期+1，只取尾巴）、全量（新票或库内不足 121 根历史）、免取（库内已最新）。严格按打印的模式与区间走下一步；若打印「全部免取」，直接在 ${dataDir} 放一个内容为 {} 的 shard_1.json 后跳到第五步。`,
        `第四步（取数，按模式二选一）：`,
        `- 单片模式：对每只 ts_code 按打印的区间调用 mcp__tushareMcp__daily（增量票只取 start_date 到今天的尾巴；全量票取近 250 个交易日日线，start_date 不晚于 today−250；保留 trade_date/open/high/low/close/vol），整理成 JSON 写到 ${dataFile}，格式为 {"<ts_code>": [{"trade_date":"YYYYMMDD","open":..,"high":..,"low":..,"close":..,"vol":..}, ...], ...}；行情只写文件，不要把数据贴进你的回复。`,
        `- 分片模式（必须多 agent）：对打印出的每个分片各派一个子代理（subagent）并行执行（无需清空 ${dataDir} 目录：脚本按 trade_date 幂等合并，残留分片重复合并无副作用）。每个子代理的任务：对片内每只 ts_code 按打印标注的区间调 mcp__tushareMcp__daily（增量票只取尾巴；全量票取近 250 个交易日日线，start_date 不晚于 today−250；保留 trade_date/open/high/low/close/vol），整理成 {"<ts_code>": [...]} 的 JSON，用写文件工具原样写入 ${dataDir}/shard_<片号>.json；子代理的回复只允许一行回执（形如「shard 1：完成 10/10，失败 []」），不得包含任何行情内容。你身为主代理要把全部分片一次性并行派发，然后只收集回执；某片失败就原样重派一次，仍失败则记录缺口后继续（覆盖率不足时脚本会 fail-closed）。`,
        `- 若本会话（含子代理）没有可用的 tushare MCP（无法调用 mcp__tushareMcp__daily），直接如实报告"无 tushare MCP，无法取数"，不要回退 akshare、不要编造行情。`,
        `第五步：单片模式跑 python scripts/momentum_strategy.py --watchlist ${watchlist} --state ${stateFile} --data ${dataFile}${extraText}；分片模式跑 python scripts/momentum_strategy.py --watchlist ${watchlist} --state ${stateFile} --data ${dataDir}${extraText}（--data 传目录时脚本自动合并全部 *.json 分片、按日期幂等写回行情库 output/quotes-store/，再用库内全量历史计算）。`,
        `该脚本为纯标准库自包含脚本，无需安装任何依赖；用法已在第五步命令中完整给出，无需阅读或分析脚本源码，直接执行即可。`,
