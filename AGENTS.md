...
output/quotes-store/        # 本地 CSV 行情库（gitignore，每标的一份 <ts_code>.csv；w-bottom-screener / momentum-rotation 共享：--plan 按库内最后交易日只取增量，--data 幂等合并写回后用全量历史计算）
...
.agents/skills/             # ZCode 技能：prosperity-analysis / daily-update / stock-valuation / portfolio-tracker / bili-video-summary / copy-trade / w-bottom-screener / momentum-rotation / watchlist-manager / workspace-init
```
