...
- **agent 侧可选 tushare MCP**：`.agents/mcp.json` 注册 tushare MCP（工具 `mcp__tushareMcp__*`），供估值/持仓/景气分析在会话内直接取真实行情与财务；该文件含密钥、已被 `.gitignore` 忽略，**不入库**。字段映射与口径局限见 `stock-valuation/references/data-source.md`。
- 联网拿到的宏观/行业数据标注数据时点。
...
- `.agents/mcp.json` 是本机 tushare MCP 配置、含 tushare token（密钥），已被 `.gitignore` 忽略，**不提交**（他人需自行用自己 token 复现）。
