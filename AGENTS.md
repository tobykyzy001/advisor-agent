...
output/watchlist/           # 观察仓清单（gitignore，w-bottom-screener 筛选的标的池）
output/w-bottom/            # W底筛选取数缓存与报告（gitignore，w-bottom-screener 运行时数据）
.agents/skills/             # ZCode 技能：prosperity-analysis / daily-update / stock-valuation / portfolio-tracker / bili-video-summary / copy-trade / w-bottom-screener / workspace-init
```
